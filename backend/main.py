...
def seed_mock_data(session: Session = Depends(get_session)):
    """Endpoint removed: seeding mock data via HTTP is disabled in this deployment."""
    raise HTTPException(status_code=404, detail="Seeding mock data via this endpoint has been disabled. Use backend scripts or local seed tools instead.")

@app.post("/admin/reset-data")
def reset_all_data(confirm: bool = False, session: Session = Depends(get_session)):
    """Endpoint removed: resetting data via HTTP is disabled in this deployment."""
    raise HTTPException(status_code=404, detail="Resetting data via this endpoint has been disabled. Use backend scripts or local reset tools instead.")
