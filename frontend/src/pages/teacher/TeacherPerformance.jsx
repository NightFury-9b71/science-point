...
import { ArrowLeft, TrendingUp, TrendingDown, Award, Users, BookOpen, Calendar, BarChart3, PieChart, Filter } from 'lucide-react'
import Card from '../../components/Card'
import Button from '../../components/Button'
import { Select } from '../../components/Form'
import { useAuth } from '../../contexts/AuthContext'
import { useTeacherClasses, useTeacherStudents, useTeacherSubjects, useExamResults, useAttendance } from '../../services/queries'
...
const TeacherPerformance = () => {
  ...
  const filteredStudents = useMemo(() => {
    if (!selectedClass) {
      // If no class selected, show all students from teacher's classes
      return myStudents
    }
    return myStudents.filter(student => student.class_id === parseInt(selectedClass))
  ...
  const performanceData = useMemo(() => {
    const studentPerformance = filteredStudents.map(student => {
      ...
      const studentResults = examResults.filter(result =>
        ...
        teacherSubjectIds.includes(result.exam?.subject_id)
      )
  ...
  }, [filteredStudents, examResults, attendanceRecords, myClasses, mySubjects])
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center space-y-3 sm:space-y-0">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h2 className="text-xl sm:text-2xl font-bold text-gray-900">Student Performance</h2>
            <p className="text-sm text-gray-600 mt-1">Performance analytics for your classes</p>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Select
            ...
            options={[
              ...
              ...myClasses.map(cls => ({ value: cls.id, label: cls.name }))
            ]}
            className="text-base"
          />
          <Select
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Card className="p-6 shadow-lg border-0 bg-gradient-to-br from-yellow-50 to-yellow-100">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className="text-sm font-medium text-yellow-600">Top Performers</p>
        ...
        <Card.Content className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="text-center">
              ...
              <p className="font-semibold text-red-700">Needs Attention (&lt;70%)</p>
              <p className="text-sm text-gray-600">
                {performanceData.classStats.totalStudents > 0
          ...
          <div className="flex items-center justify-between">
            ...
            {selectedClass && (
              <div className="text-sm text-gray-600">
                Class: {myClasses.find(cls => cls.id === parseInt(selectedClass))?.name}
              </div>
            )}
          </div>
        </Card.Header>
