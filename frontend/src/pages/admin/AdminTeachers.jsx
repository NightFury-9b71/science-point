...
import { 
  Plus, Users, Edit, Briefcase, Eye, ArrowLeft, Search, KeyRound, CheckCircle, 
  Download, FileText, Copy, AlertCircle, User, Hash, Phone, Mail, DollarSign, 
  Calendar, BookOpen, Shield
} from 'lucide-react'
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <ViewModal
        ...
        data={selectedTeacher ? {
          ...selectedTeacher,
          photo_url: selectedTeacher.user?.photo_url || selectedTeacher.user?.photo_path,
          full_name: selectedTeacher.user?.full_name || 'Teacher Profile',
          subtitle: `Employee ID: ${selectedTeacher.employee_id}`,
          details: `${selectedTeacher.salary || 200} BDT per class • Teacher ID: #${selectedTeacher.id}`,
          verified: selectedTeacher.user?.is_active,
          badges: [
            '🎓 Teacher',
            selectedTeacher.user?.is_active ? 'Active' : 'Inactive',
            `Salary: ${selectedTeacher.salary || 200} BDT`
          ].filter(Boolean)
        } : null}
        sections={[
          {
            ...
            fields: [
              { 
                key: 'name', 
                label: 'Full Name', 
                value: selectedTeacher?.user?.full_name
              },
              { 
                key: 'username', 
                label: 'Username', 
                value: selectedTeacher?.user?.username
              },
              { 
                key: 'employeeId', 
                label: 'Employee ID', 
              ...
              { 
                key: 'phone', 
                label: 'Phone', 
                value: selectedTeacher?.user?.phone
              },
              { 
                key: 'email', 
                label: 'Email', 
          ...
          },
          {
            ...
            fields: [
              { 
                key: 'salary', 
                label: 'Salary per Class', 
                value: `${selectedTeacher?.salary || 200} BDT`
              },
              {
                key: 'teacherId',
                label: 'Teacher ID',
